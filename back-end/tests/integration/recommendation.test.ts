...
describe("Test GET /recommendations/:id", () => {
  it("Should return status 200 and return recommendations", async () => {
    ...
    const { body: recommendations } = await supertest(app).get(
      "/recommendations"
    );
...
describe("Test POST  /recommendations/:id/upvote", () => {
  it("Should return status 200 and add vote to recommendation", async () => {
    ...
    const result = await supertest(app).post(
      `/recommendations/${recommendations[0].id}/upvote`
    );

    const updatedRecommendation = await supertest(app).get(
      `/recommendations/${recommendations[0].id}`
    );

    expect(result.status).toEqual(200);
    expect(updatedRecommendation.body.score).toEqual(
...
describe("Test GET /:id/downvote", () => {
  it("Should return status 200 and decrease vote to recommendation", async () => {
    ...
    const result = await supertest(app).post(
      `/recommendations/${recommendations[0].id}/downvote`
    );
